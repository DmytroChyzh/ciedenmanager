...
export default function GraphCard({ title, value, data, type = 'line', icon, percent, color = '#651FFF' }: GraphCardProps) {
  const up = percent >= 0;
  return (
    <div className="bg-white rounded-2xl border border-[#ede7ff] flex flex-col p-8 min-w-[260px]">
      ...
      <div className="w-full min-h-[150px] animate-fadeIn">
        <ResponsiveContainer width="100%" height={150}>
          {type === 'bar' ? (
            <BarChart data={data} margin={{ top: 8, right: 0, left: 0, bottom: 0 }}>
              ...
              <YAxis domain={[0, 20]} ticks={[0,5,10,15,20]} interval={0} axisLine={false} tickLine={false} tick={{ fontSize: 12, fill: '#8B97B0', fontWeight: 500 }} />
